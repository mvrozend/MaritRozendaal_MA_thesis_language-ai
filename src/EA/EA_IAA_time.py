...
def filter_conversation_strict(conv_df, file):
    ...
    for idx, (_, row) in enumerate(conv_df.iterrows()):
        ...
        for cat_idx, (cat, level) in enumerate(zip(categories, levels)):
            ...
            try:
                segment_df["segment_time"] = row["relative_time"][categories.index(cat)]
            except:
                segment_df["segment_time"] = "None"
...
def calculate_similarity(baseline_df, additional_df, annotator, sl):
    ...
    if sl == "strict": 
        ...
        additional_df["relative_time"] = additional_df["relative_time"].apply(literal_eval)
    ...
    for segment_id in baseline_df["segment_id"].unique():
        ...
        baseline_time = segment["segment_time"].iloc[0]
        ...
        for _, row in comparison.iterrows():
            if category in row["categories"]:
                ...
                try:
                    additional_time = row["relative_time"][i]
                except:
                    additional_time = "None"
                break
        if additional_time in ["None", None]:
            score = None
        elif additional_time == baseline_time:
            score = 1
        else:
            score = 0
